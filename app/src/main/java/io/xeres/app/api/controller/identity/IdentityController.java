...
public class IdentityController
{
	...
	@Operation(summary = "Return an identity's image by GxsId (autogenerated)")
	@ApiResponse(responseCode = "200", description = "Request successful")
	public ResponseEntity<InputStreamResource> downloadImageByGxsId(@RequestParam(value = "gxsId") String gxsId)
	{
		var image = identiconService.getIdenticon(GxsId.fromString(gxsId).getBytes());
		return ResponseEntity.ok()
				.contentLength(image.length)
				.contentType(ImageDetectionUtils.getImageMimeType(image))
				.body(new InputStreamResource(new ByteArrayInputStream(image)));
