...
public class DiscoveryRsService extends RsService
{
	...
	private void sendContact(Location toLocation, Location aboutLocation, SocketAddress toLocationAddress)
	{
		...
		builder.setLastContact(aboutLocation.getLastConnected() != null ? aboutLocation.getLastConnected() : Instant.now()); // RS uses Instant.now() XXX: find out if there is any issue with that change. it tells since how long we've been connected
